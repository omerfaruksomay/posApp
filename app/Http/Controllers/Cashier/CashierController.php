    ...
    private function getSaleDetails($sale_id)
    {
        ...
        foreach ($saleDetails as $saleDetail) {
            ...
            if ($saleDetail->status == "noConfirm") {
                ...
                $html .= '<td><a data-id="' . $saleDetail->id . '" class="btn btn-danger btn-delete-saledetail">Not Confirmed</a></td>';
            } else { // status == "confirm"
        ...
        if ($showBtnPayment) {
            $html .= '<button data-id="' . $sale_id . '" data-totalAmount="' . $sale->total_price . '" class="mb-4 btn btn-success btn-block btn-payment" data-toggle="modal" data-target="#exampleModal">Payment</button>';
        } else {
            $html .= '<button data-id="' . $sale_id . '" class="btn btn-warning btn-block btn-confirm-order mb-4">Confirm Order</button>';
        }
